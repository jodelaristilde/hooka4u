...
export default function NewOrder() {
  ...
  const [resetCountdown, setResetCountdown] = useState(8)
    ...
    const interval = setInterval(() => {
      setResetCountdown((prev) => {
        if (prev <= 1) {
          ...
          window.location.href = "/place-new-order"
          return 0
        }
        return prev - 1
      })
        ...
        <DialogContent className="sm:max-w-md bg-zinc-900 text-white border-zinc-800">
          ...
          <div className="flex flex-col gap-4 py-4">
            ...
            <p className="text-xs text-zinc-500 text-center">
              Returning to the welcome screen in {resetCountdown} seconds...
            </p>
            <Button
              onClick={() => { window.location.href = "/place-new-order" }}
